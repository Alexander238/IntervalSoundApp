...
class MainScreen extends StatefulWidget {
  ...
  final Function(TimerData)? onSave;

  const MainScreen({Key? key, required this.timerData, this.onSave})
      : super(key: key);
...
class _MainScreenState extends State<MainScreen> {
  ...
  void initState() {
    super.initState();
    _selectedTimeMinutes = widget.timerData.minutes;
    _selectedTimeSeconds = widget.timerData.seconds;
  ...
  void _handleTextFieldChanged(TextEditingController controller,
    ...
    widget.timerData.minutes = _selectedTimeMinutes;
    widget.timerData.seconds = _selectedTimeSeconds;
  ...
  void _startTimer() {
    ...
    // Total number of steps for the progress update
    const totalSteps = 100;
    // Duration of each tick in milliseconds
    final tickDuration = _interval * 1000 / totalSteps;
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        Navigator.pop(context, widget.timerData);
        return true;
      },
      child: SafeArea(
        child: Scaffold(
            ...
            child: Center(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  heightDivider(
                      context, MediaQuery.of(context).size.height * 0.02),
                  ...
                  heightDivider(
                      context, MediaQuery.of(context).size.height * 0.03),
                  // Seconds input field
                  SizedBox(
                    width: MediaQuery.of(context).size.width * 0.6,
                    child: TextField(
